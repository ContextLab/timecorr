...
cdef np.ndarray coefficients, activations_sum, coefficients_sum, c_activations
cdef int time_len, subj_num, activations_len
...
def isfc(double[:,:,:] multi_activations, int gaussian_variance):
    ...
    cdef int timepoint, subj
    cdef np.ndarray[double, ndim=2] correlations_vector
    cdef np.ndarray[double, ndim=3] correlations_mean
    cdef np.ndarray[double, ndim=4] correlations
    cdef np.ndarray gaussian_array, coefficient
    ...
    def isfc_subject_helper(subj):
        cdef np.ndarray[double, ndim=2] normalized_activations, normalized_sum_activations
        cdef np.ndarray[double, ndim=3] corelation
        cdef np.ndarray sigma_activations, sigma_activations_sum
        cdef int timepoint

        # correlation = np.zeros([time_len,activations_len,activations_len])

        def isfc_timepoint_helper(timepoint):
            corrs = np.zeros([activations_len,activations_len])
            ...
            for i in range(activations_len):
                for j in range(activations_len):
                    corrs[i,j] = np.sum(np.multiply(np.multiply(coefficients[timepoint,0], normalized_activations[i]), normalized_sum_activations[j]))/(sigma_activations[i]*sigma_activations_sum[j]*coefficients_sum[timepoint])
            return corrs

        correlation = np.array(map(isfc_timepoint_helper, range(time_len)))
        return correlation

    #calculate the correlations for each timepoint for each subject
    # p = Pool(10)
    correlations = np.array(map(isfc_subject_helper,range(subj_num)))
    # p.terminate()
...
def wcorr(activations, gaussian_variance):
    # #cython variable declaration
    cdef int time_len, activations_len
    cdef np.ndarray[double, ndim=2] correlations_vectors
    ...
    def wcorr_helper(timepoint):
        cdef np.ndarray sigma, coefficient, correlations_vector
        cdef np.ndarray[double, ndim=2] coefficient_tiled, normalized_activations
        cdef int  i, j, index
